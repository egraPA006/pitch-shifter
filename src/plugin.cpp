...
struct Chan {
  ...
  void step(float *out, double ratio) {
    ...
    if (gap < MIN_GAP)
      rd -= static_cast<double>(HOP);
    if (gap > MAX_GAP)
      rd += static_cast<double>(gap - MIN_GAP);
...
static bool plugin_activate(const clap_plugin_t *p, double, uint32_t,
                            uint32_t) {
  auto *s = self(p);
  for (auto &c : s->ch)
    c = Chan{};
...
static bool par_get(const clap_plugin_t *p, clap_id id, double *v) {
  if (id == PARAM_SEMITONES) {
    *v = self(p)->semitones;
    return true;
  }
  return false;
}
static bool par_to_text(const clap_plugin_t *, clap_id id, double v, char *buf,
                        uint32_t sz) {
  if (id == PARAM_SEMITONES) {
    snprintf(buf, sz, "%.2f st", v);
    return true;
  }
  return false;
}
static bool par_from_text(const clap_plugin_t *, clap_id, const char *,
                          double *) {
  return false;
}
...
static const clap_plugin_descriptor_t k_desc = {
    ...
    .features = (const char *[]){CLAP_PLUGIN_FEATURE_AUDIO_EFFECT,
                                 CLAP_PLUGIN_FEATURE_PITCH_SHIFTER, nullptr},
};

static uint32_t fac_count(const clap_plugin_factory_t *) { return 1; }
static const clap_plugin_descriptor_t *fac_desc(const clap_plugin_factory_t *,
                                                uint32_t i) {
...
static const clap_plugin_factory_t s_factory = {fac_count, fac_desc,
                                                fac_create};
